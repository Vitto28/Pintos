...
static bool compare_priority(const struct list_elem * i1, const struct list_elem * i2, void * aux);
static int calculate_priority();
static void check_highest_priority();
...
void thread_tick(void) {
    ...
    if (thread_mlfqs){
        ...
        if (timer_ticks() % TIMER_FREQ == 0) {
            ...
            FPReal op1 = FPR_MUL_INT(load_avg, 59);
            FPReal op2 = FPR_DIV_INT(op1, 60);
            int increment = (thread_current() != idle_thread) ? 1 : 0;
            FPReal op3 = INT_DIV_INT(list_size(&ready_list) + increment, 60);
            load_avg = FPR_ADD_FPR(op2, op3);
...
static int calculate_priority(FPReal recent_cpu, int nice){
    return PRI_MAX - FPR_TO_INT(FPR_DIV_INT(recent_cpu, 4)) - nice * 2;
}

void update_recent_cpu(struct thread * t, void * aux UNUSED){
    // (2 * load_avg) / (2 * load_avg + 1) * recent_cpu + nice
    FPReal op1 = FPR_MUL_INT(load_avg, 2);
    FPReal op2 = FPR_MUL_FPR(op1, t->recent_cpu);
    FPReal op3 = FPR_ADD_INT(op1, 1);
    FPReal op4 = FPR_DIV_FPR(op2, op3);
    t->recent_cpu = FPR_ADD_INT(op4, t->nice);
}
...
void thread_set_nice(int nice) { 
    ...
    thread_set_priority(calculate_priority(thread_current()->recent_cpu, nice));
}
...
/* Compares the priority of two threads and is used to sort the list of ready threads in 
   decreasing fashion. It also handles the case in which multiple threads have the highest priority
   and it introduces the round robin rotation of those threads. */
static bool compare_priority(const struct list_elem * i1, const struct list_elem * i2, void * aux UNUSED) {
    struct thread * ia = list_entry(i1, struct thread, elem);
    struct thread * ib = list_entry(i2, struct thread, elem);
